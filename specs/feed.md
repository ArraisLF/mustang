...
- **List feed items** (public): All items ordered by timestamp descending, includes like count and current user's like status
...
### Frontend
- `Feed` component fetches and renders all feed items
- `FeedPost` component renders individual post with media gallery, like/share/delete actions
- `CreatePostForm` component with media management (add, remove, reorder, type toggle, URL validation)
